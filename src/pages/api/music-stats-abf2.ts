...
function readDistinctTrackCount(payload: any): number {
  const row = Array.isArray(payload?.data)
    ? payload.data[0] || {}
    : payload?.data || {};

  const raw =
    row?.countDistinct?.track_key ??
    row?.countDistinct?.["track_key"] ??
    0;
...
export const GET: APIRoute = async () => {
  try {
    ...
    const [tracksJson, playsJson] = await Promise.all([
      directusFetch(
        `/items/plays_abf2?aggregate[countDistinct]=track_key`
      ),
      directusFetch(
        `/items/plays_abf2?aggregate[count]=*`
      ),
    ]);

    const tracks = readDistinctTrackCount(tracksJson);
    const airplays = readCount(playsJson);
